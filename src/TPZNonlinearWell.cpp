...
void TPZNonlinearWell::Contribute(const TPZVec<TPZMaterialDataT<STATE>> &datavec, REAL weight, TPZFMatrix<STATE> &ek,
    ...
    //Tangent matrix
    REAL factor = fC * weight * (pow(std::abs(Qsol), 3. / 4.) + 3. / 4. * pow(std::abs(Qsol), 3. / 4.));
    ...
    factor = -fKvw * weight;
    ek.AddContribution(nphiQ, nphiQ, phiP, 0, phiP, 0, factor); // C

    // Residual vector constitutive equation (negative)
    factor = fC * weight * Qsol * pow(std::abs(Qsol), 3. / 4.);
    ef.AddContribution(0, 0, phiQ, 1, Aux, 0, -factor);
    factor = -psol * weight;
...
int TPZNonlinearWell::VariableIndex(const std::string &name) const {
    ...
    if (!strcmp("DivFlux", name.c_str())) return 3;
    if (!strcmp("GradPressure", name.c_str())) return 4;
    DebugStop();
    return -1;
}
