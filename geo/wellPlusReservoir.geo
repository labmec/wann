...
// Meshing parameters
theta_div = 5;    // Azimuthal division (of each circle quarter)
radial_div = 8;   // Radial division of the wellbore
p_res = 1.8;      // Progression coefficient of the near-well region mesh
p_well = 0.3;     // Progression coefficient of the wellbore mesh

axial_div = Floor((theta_div-1)*(Lnw+Hnw)/Hnw);
s = Sin(Pi/4.);
...
Physical Surface("surface_farfield",106) = {77,78,80,82};
Physical Surface("surface_cap_rock",107) = {79,81}; 
Physical Point("point_heel",108) = {2};
Physical Point("point_toe",109) = {6};
Physical Volume("volume_reservoir",110) = {vl1,vl2,vl3,vl4,vl5,vl6,v9};
...
Coherence Mesh;
Save "../input/wellPlusReservoir.msh";